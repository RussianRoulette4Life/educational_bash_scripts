...
## Понять, "сорснут" скрипт или нет можно через сравнение с $BASH_SOURCE

if [[ $0 != $BASH_SOURCE ]]; then
	echo "скрипт был сорснут!"
	script_dir="$(dirname $BASH_SOURCE)"
else
	echo "скрипт был вызван!"
...
echo "Буду далее записывать в $OUTPUT_FILE"
...
## только после ". 1st.sh" или "source 1st.sh", обычно исполнение не сработает. В целом работает
## как копировать/вставить - что вы в alias записали, то, по сути, будет занесено в терминал
## при вызове псевдонима

alias run_1st_again=". $abs_script_path"

### далее я использую обратный слэш (бэкслэш, \) для отличия кавычек, которые объединяют команды
### под псевдоним "hi", от кавычек, которые я хочу, чтобы во время "исполнения" hi были выписаны
### в терминал как кавычки, а следовательно были приняты echo и grep. обычно для решения этой
### проблемы используют одинарные кавычки для hi и двойные внутри, но я предпочитаю, когда возможно,
### использовать двойные, поскольку только они позволяют внутрь строк заносить значения из перемен
### окружения.
